...
public class BookmarkServiceImpl implements BookmarkService {
    ...
    public BookmarkResponse createBookmark(BookmarkRequest bookmarkRequest) {
        ...
        Work work = workRepo.findById(bookmarkRequest.getWorkId()).orElseThrow(
                () -> new ResourceNotFoundException("Work with id: " + bookmarkRequest.getWorkId() + " not found"));
        Chapter chapter = chapterRepo.findById(bookmarkRequest.getChapterId())
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Chapter with id: " + bookmarkRequest.getChapterId() + " not found"));

        // Deduplicate: If bookmark already exists for this user and work, return it.
        var existing = bookmarkRepo.findAllByUserAndWork(user, work);
        if (!existing.isEmpty()) {
            return bookmarkMapper.toBookmarkResponse(existing.get(0));
        }
    ...
    public BookmarkResponse getBookmark(Long id) {
        ...
        Bookmark bookmark = bookmarkRepo.findByIdAndUser(id, user)
                .orElseThrow(() -> new ResourceNotFoundException("Bookmark with id: " + id + " not found"));
