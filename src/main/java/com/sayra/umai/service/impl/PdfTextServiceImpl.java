...
public class  PdfTextServiceImpl implements PdfTextService {
  ...
  public List<ChapterData> extractFromOutline(PDDocument document, PDDocumentOutline outline) throws IOException {
    ...
    while (current != null) {
      ...
      if (!text.isEmpty()) {
        ...
        chapters.add(new PdfServiceImpl.ChapterData(chapterNumber++, title, type_html, chunks));
  ...
  @Override
  public PDPage resolveDestinationPage(PDOutlineItem item, PDDocument document) {
  ...
  public List<ChapterData> extractChapters(File pdfFile) throws IOException {
    try (PDDocument document = Loader.loadPDF(pdfFile)) {
      ...
      if (outline != null) {
        List<PdfServiceImpl.ChapterData> chapters = extractFromOutline(document, outline);
  ...
  @Override
  public List<String> chunkTextToHtml(String text, int maxBytes) {
  ...
  @Override
  public PDPage lookupNamedDestinationPage(String name, PDDocument document) {
    try {
  ...
  @Override
  public List<ChapterData> extractByTextPatterns(File pdfFile) throws IOException {
    String fullText;
    try (PDDocument document = Loader.loadPDF(pdfFile)) {
      PDFTextStripper stripper = new PDFTextStripper();
    ...
    if (positions.isEmpty()) {
      ...
      chapters.add(new PdfServiceImpl.ChapterData(1, "Автоматическая глава", type_html, chunks));
      return chapters;
    }
    ...
    for (int i = 0; i < positions.size(); i++) {
      ...
      if (!body.isEmpty()) {
        ...
        chapters.add(new PdfServiceImpl.ChapterData(i + 1, title, type_html, chunks));
